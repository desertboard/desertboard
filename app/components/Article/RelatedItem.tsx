...
const RelatedItem = ({listData}:ListItemProps) => {
  return (
    <div key={listData._id} className="flex flex-col">
      ...
      <p className="text-[#151515BF] text-[14px] uppercase font-bold nuber-next-heavy pt-5 pb-0 md:pb-4">{formatDate(listData.date)}</p>
     
      <div className='flex flex-col gap-5 md:gap-6git'>
      <h3 className="text-[24px] font-bold nuber-next-heavy text-[#002D28] max-w-[45ch] overflow-hidden text-ellipsis display-webkit-box line-clamp-2 webkit-box-orient-vertical">{listData.title}</h3>
       {/*  <div className="flex items-baseline gap-2 pt-4 ">
          ...
          {
            <ul className="news__category list-none text-black uppercase font-bold text-font14 leading-normal flex flex-wrap gap-3 opacity-75">
