...
const UspItem = ({ uspTitle, order, mainImg, uspDesc, uspIcon, onMouseEnter,iconAlt,imageAlt }: UspProps) => {
    return (
      <div className={`usp-item usp-${order} relative overflow-hidden hover:pb-7 group`} onMouseEnter={onMouseEnter}>
        <div className="usp-item__bg absolute inset-0 overflow-hidden">
          <Image src={mainImg} alt={imageAlt!=="" ? imageAlt : "uspImage"} className="w-full h-full object-cover" fill></Image>
        </div>
        <div className="usp-item__i absolute top-5 left-5 xl:top-10 xl:left-10 z-20">
          <Image src={uspIcon} alt={iconAlt!=="" ? iconAlt : "icon"} className="" width={40} height={40}></Image>
        </div>
