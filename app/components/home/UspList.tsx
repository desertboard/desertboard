        ...
        <div className={`usp__list ${activeOrder ? "active-" + activeOrder : ""}`}
           >
            {data?.home[0]?.thirdSection.contents.map((item,index)=>(
              <UspItem uspTitle={item.title} onMouseEnter={() => setActiveOrder((index+1).toString())} iconAlt={item.logoAlt} imageAlt={item.imageAlt} uspIcon={item.logo} uspDesc={item.description} order={(index+1).toString()} mainImg={item.image} key={index}/>
            ))}
