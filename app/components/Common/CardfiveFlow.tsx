    ...
    variants={{
      ...
      {data.map((framework) => (
        <div className="relative group bg-gray-800 csmn overflow-hidden sectorcrd" key={framework.id}>
          <Image src={framework.image} alt={framework.title} className="w-full h-[340px] lg:h-[360px] xxl:h-[400px] 3xl:h-[460px] object-cover opacity-80 group-hover:opacity-100 transition-all duration-300" />
          ...
          <div className="absolute top-0 text-white w-full h-full transition-all duration-500 ease-in-out p-5 lg:p-8 ">
            ...
            <p className="mt-5 w-full lg:opacity-0 group-hover:opacity-75   group-hover:w-full transition-opacity duration-500  delay-100 text-font18 leading-[1.5]" dangerouslySetInnerHTML={{ __html: formatText(framework.dec) }} />
