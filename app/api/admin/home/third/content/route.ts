...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if(home){
        console.log(home)
        const toBeEditedItem = home.thirdSection.contents.find((item:{_id:string})=>item._id==id)
        if(toBeEditedItem){
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if(home){
        console.log(home)
        const toBeDeletedItem = home.thirdSection.contents.findIndex((item:{_id:string})=>item._id==id)
        if(toBeDeletedItem!==-1){
            home.thirdSection.contents.splice(toBeDeletedItem,1)
            await home.save()
            return NextResponse.json({ message: "Content updated successfully" }, { status: 200 });
        }else{
            return NextResponse.json({ message: "Content updation failed" }, { status: 404 });
